...
int init_7841(void) ; 
void init_can(void) ;
void envoi_periodique(long arg) ; 
void reception_periodique(long arg) ; 
void envoi_can(int id, int DLC, void *msg);
void reception_can(void) ;
static int caninit(void) ;
static void canexit(void) ;
...
void envoi_can(int id, int DLC, void *msg) //Mettre les variable en miniscule et define en miniscule
	   {
...

     {
	    ...
	    //********************************* Lecture du DLC  ********************************************************//    
			...
			for (i=0; i<= DLC;i++)
	      	...
	      	printk("La donnee recu est %p /n",data);
...
static int caninit(void) {
  ...
  RTIME now;
   ...
   /* creation taches périodiques d'envoi */
   rt_set_oneshot_mode();
   ierr = rt_task_init(&ma_tache,envoi_periodique,0,STACK_SIZE, PRIORITE, 0, 0); 
  ...
  /* creation taches périodiques de reception */
   ierrr = rt_task_init(&ma_reception,reception_periodique,0,STACK_SIZE, PRIORITE2, 0, 0); 
