...
static RT_TASK ma_tache_send, ma_tache_receive,ma_tache_identite ;
...
int caninit(void) {
  ...
  RTIME now;
   ...
   ierr = rt_task_init(&ma_tache_receive,reception_can,0,STACK_SIZE, PRIORITE, 0, 0); 
   ierrr = rt_task_init(&ma_tache_identite,identite,0,STACK_SIZE, PRIORITE, 0, 0);
   
   start_rt_timer(nano2count(TICK_PERIOD));
   now = rt_get_time();
   
   rt_task_make_periodic(&ma_tache_send, now, nano2count(PERIODE_CONTROL));
   rt_task_make_periodic(&ma_tache_receive, now, nano2count(TICK_PERIOD));
   rt_task_make_periodic(&ma_tache_identite, now, nano2count(PERIODE_CONTROL*2));
...
void canexit(void) {
  ...
  rt_task_delete(&ma_tache_identite);
}
